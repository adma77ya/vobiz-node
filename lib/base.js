  ...
  executeAction(task = '', secondaryTask = '', method = 'GET', params = {}, action, secondaryAction) {
    ...
    return new Promise((resolve, reject) => {
      client(method, action + task + '/' + secondaryAction + secondaryTask + '/', params)
        .then(response => {
                    resolve(new VobizGenericResponse(response.body, secondaryIdField));
        })
        .catch(error => {
          reject(error);
        });
...
module.exports = {
  VobizGenericResponse,
  VobizResource,
  VobizSecondaryResource,
